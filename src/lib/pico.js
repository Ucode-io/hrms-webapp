...
/*
 * Вендорим, а не ставим пакет `picojs`: тот лежит на npm как CommonJS с двумя
 * присваиваниями необъявленным глобалям (`pico = {}` и счётчик `i` в
 * instantiate_detection_memory). Модули ES всегда strict, поэтому после сборки
 * он падает ReferenceError — проверено на esbuild. Правки здесь ровно три:
 * `const pico`, `let i`, `export default` вместо `module.exports`.
 * Прогон — `node src/lib/pico.check.mjs`.
 */
const pico = {}
